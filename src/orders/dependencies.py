...
class OrderRepository:
    ...
    @classmethod
    async def find_by_id(cls, order_id: int) -> SOrder | None:
        async with AsyncSessionLocal() as session:
    ...
    @classmethod
    async def add_order(cls, order_data: SOrderAdd) -> int | None:
        async with AsyncSessionLocal() as session:
            order_id = await OrderService.check_and_create_order(order_data, session)
            return order_id

    @classmethod
    async def update_status(cls, order_id: int, new_status: SOrderStatus) -> SOrder | None:
        async with AsyncSessionLocal() as session:
